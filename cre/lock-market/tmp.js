...
var configSchema = exports_external.object({
  ...
  marketAddress: exports_external.string(),
  outcome: exports_external.number().int().min(0).max(2)
});
var postData = (sendRequester, config) => {
  const dataToSend = {
  ...
  let responseData;
  try {
    const bodyBytes2 = resp.body instanceof Uint8Array ? resp.body : new Uint8Array(Object.values(resp.body));
    const responseBody = new TextDecoder().decode(bodyBytes2);
...
var onCronTrigger = (runtime2) => {
  ...
  const result = httpClient.sendRequest(runtime2, postData, consensusIdenticalAggregation())(runtime2.config).result();
  if (result.response.success) {
    runtime2.log(`Successfully locked market. TX: ${result.response.txHash}, Block: ${result.response.blockNumber}`);
